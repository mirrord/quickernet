...
class QuadraticCost(CostFunction):
    def forward(self, inputs, expected_output):
        return 0.5 * np.sum(np.square(inputs - expected_output)).item() / inputs.shape[0]
...
class CrossEntropyCost(CostFunction):
    def forward(self, inputs, expected_output):
        return -1*np.sum( expected_output*np.log(inputs) + (1-expected_output)*np.log(1-inputs) ).item() / inputs.shape[0]
    def backward(self, inputs, expected_output):
        return (inputs-expected_output)/(inputs*(1-inputs))

class HellingerCost(CostFunction):
    def forward(self, inputs, expected_output):
        return 0.70710678*np.sum( np.square(np.sqrt(inputs)-np.sqrt(expected_output)) ).item() / inputs.shape[0]
    def backward(self, inputs, expected_output):
