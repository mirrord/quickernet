    ...
    target = {
        ...
        "__init__": {
            ...
            "body": ["self.__node0_step1_bias = np.random.randn(1, output_dim)",
                     "self.__node0_step1_weight = np.random.randn(input_dim, output_dim) * np.sqrt(",
                     ...
                     "self.__node0_step2_weight = np.random.randn(input_dim_1, output_dim_1) * np.sqrt(",
                     "1 / (input_dim_1 + output_dim_1)",
                     ")",
    ...
    target = {
        ...
        "__init__": {
            ...
            "body": ["self.__model0_node0_step0_bias = np.random.randn(1, output_dim)",
                     "self.__model0_node0_step0_weight = np.random.randn(input_dim, output_dim) * np.sqrt(",
                     "1 / (input_dim + output_dim)",
                     ")",
        ...
        "backward": {
            ...
            "return": ["[__model0_node0_update]", "[__model0_node0_step1_gradient, __model0_node0_gradient]"],
        },
    }
...
class DGM:
    def __init__(self, input_dim, output_dim):
        ...
        self.__node0_step0_input_shape = ('BATCH_N', input_dim)
    def forward(self, inputs):
        self.__model0_first_inputs = inputs
        self.__node0_step0_out = np.dot(self.__model0_first_inputs, self.__node0_step0_weight) + self.__node0_step0_bias
        return 1 / (1 + np.exp(-self.__node0_step0_out))
    def backward(self, error_gradient):
        forward_output = 1 / (1 + np.exp(-self.__node0_step0_out))
        self.__node0_step0_gradient = error_gradient * forward_output * (1 - forward_output)
        bias_gradient = self.__node0_step0_gradient
        weight_gradient = np.dot(self.__model0_first_inputs.T, bias_gradient)
